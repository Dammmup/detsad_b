...
export class ContactInfectionJournalService {
  ...
  async create(journalData: Partial<IContactInfectionJournal>, userId: string) {
    ...
    const journal = new ContactInfectionJournal({
      ...journalData,
  ...
  async getUpcomingAppointments(days: number = 7) {
    ...
    const journals = await ContactInfectionJournal.find({
      nextAppointmentDate: {
        $gte: today,
        $lte: futureDate
      }
  ...
  async updateStatus(id: string, status: 'pending' | 'completed' | 'reviewed') {
    const journal = await ContactInfectionJournal.findByIdAndUpdate(
      id,
      { status },
      { new: true }
    ).populate('childId', 'fullName iin')
  ...
  async getStatistics() {
    const stats = await ContactInfectionJournal.aggregate([
      {
        $group: {
          _id: '$infectionType',
          count: { $sum: 1 }
    ...
    const tracedStats = await ContactInfectionJournal.aggregate([
      {
        $group: {
          _id: '$contactsTraced',
          count: { $sum: 1 }
    ...
    const total = await ContactInfectionJournal.countDocuments();
