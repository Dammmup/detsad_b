...
\`\`\`json
{
  ...
  "responseTemplate": "Телефон сотрудника успешно обновлён"
}
\`\`\`
...
5. **Шаблоны ответов**: Используй плейсхолдеры \`{count}\` для количества и \`{result}\` для списка или данных объекта. Поддерживается dot-нотация: \`{result.fullName}\`, \`{result.role}\`.
...
8. **Поля для поиска**: Для сотрудников и детей всегда запрашивай \`fullName\`, \`role\` и \`phone\` в projection, чтобы ответ был информативным.
