...
8. **Обработка отсутствия данных**: Если по запросу ничего не найдено, не придумывай данные, а ответь, что информация отсутствует.
9. **Формат JSON**: НИКОГДА не используй физические переносы строк внутри значений JSON. Используй \`\\n\`.
10. **ЗАПРЕТ ВЛОЖЕННОСТИ \$regex В \$in**: MongoDB НЕ поддерживает \`\$regex\` внутри оператора \`\$in\`. Используй \`\$or\`.
...
| active | Boolean | Активен ли сотрудник (true = работает) |
| iin | String | ИИН |
| groupId | ObjectId | Ссылка на группу |
